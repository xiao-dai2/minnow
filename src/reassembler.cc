...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring )
{
  ...
  //    return ;
  // }
  if(is_last_substring) end_index = first_index+siz;
  ...
  if(expected_index==first_index){
      ...
      if(dict.count(first_index)||expected_index+ava_capacity<first_index+siz) return;
      q.push({first_index,data});
      dict.insert(first_index);
  }
  
  if(expected_index==end_index&&end_index) output_.writer().close();
}
